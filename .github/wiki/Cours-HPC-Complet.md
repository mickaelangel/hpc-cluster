...
**Durée estimée** : ~1 h de lecture.  
**Prérequis** : bases en ligne de commande et en programmation.

### Table des matières

1. [Introduction au HPC](#1-introduction-au-calcul-haute-performance-hpc)
2. [Architecture des clusters](#2-architecture-des-clusters-hpc)
3. [Ordonnancement (scheduler)](#3-ordonnancement-des-jobs--rôle-du-scheduler)
4. [MPI et OpenMP](#4-programmation-parallèle--mpi-et-openmp)
5. [Stockage](#5-stockage-et-systèmes-de-fichiers)
6. [GPU](#6-gpu-et-accélération)
7. [Conteneurs](#7-conteneurs-et-hpc)
8. [Sécurité](#8-sécurité-et-bonnes-pratiques)
9. [Monitoring](#9-monitoring-et-observabilité)
10. [Synthèse et suite](#10-synthèse-et-suite)
...
- **Loi d’Amdahl** : la partie séquentielle du code limite le speedup maximal ; paralléliser le plus possible les parties coûteuses.
...
- **HPC** = parallélisme (données, tâches, mémoire partagée OpenMP, mémoire distribuée MPI) + architecture (frontaux, calcul, stockage, interconnect) + scheduler (Slurm) + bonnes pratiques (I/O, GPU, conteneurs, sécurité).
- **Speedup et efficacité** caractérisent la scalabilité ; la loi d’Amdahl limite le gain.
- **Slurm** : soumission (`sbatch`, `srun`), file d’attente, partitions, QoS ; cycle de vie PENDING → RUNNING → COMPLETED/FAILED.
- **Stockage** : home (NFS), scratch (parallèle, purge), Lustre/BeeGFS/Ceph pour le gros I/O.
- **GPU** : ressource gérée par le scheduler (`--gres=gpu`) ; Singularity/Apptainer pour conteneurs sur les nœuds.

### Pour aller plus loin dans ce projet

- **[Premiers Pas](Premiers-Pas.md)** : connexion, premier job, premiers commandes
...
- **Manuels Vol. 1–8** : voir [Sommaire du Manuel HPC](Manuel-HPC-Sommaire-Complet.md) pour le détail par thème (réseau, Lustre, performances, observabilité, etc.)
