# Wiki - Cluster HPC Enterprise

> **Documentation collaborative pour le Cluster HPC Enterprise**

## 📋 Navigation

### Pages Principales

- **[FAQ](.github/wiki/FAQ.md)** : Questions fréquentes
- **[Installation Rapide](.github/wiki/Installation-Rapide.md)** : Guide d'installation condensé
- **[Configuration de Base](.github/wiki/Configuration-de-Base.md)** : Configuration minimale
- **[Dépannage](.github/wiki/Depannage.md)** : Solutions aux problèmes courants
- **[Astuces](.github/wiki/Astuces.md)** : Trucs et astuces
- **[Cas d'Usage](.github/wiki/Cas-d-Usage.md)** : Exemples d'utilisation
- **[Accueil wiki complet](.github/wiki/Home.md)** : Sommaire formation HPC, cours, manuels, glossaire

## 🔗 Liens Utiles

### Documentation Principale

- **📚 [Documentation Complète](https://github.com/mickaelangel/hpc-cluster/tree/main/docs)** : 93 guides disponibles
- **📖 [Index Documentation](https://github.com/mickaelangel/hpc-cluster/blob/main/docs/INDEX_DOCUMENTATION.md)** : Navigation dans tous les guides
- **🔍 [Guide Troubleshooting](https://github.com/mickaelangel/hpc-cluster/blob/main/docs/GUIDE_TROUBLESHOOTING.md)** : Solutions aux problèmes

### Support

- **💬 [Discussions GitHub](https://github.com/mickaelangel/hpc-cluster/discussions)** : Poser des questions
- **🐛 [Signaler un Bug](https://github.com/mickaelangel/hpc-cluster/issues/new?template=bug_report.md)** : Créer une issue
- **✨ [Demander une Fonctionnalité](https://github.com/mickaelangel/hpc-cluster/issues/new?template=feature_request.md)** : Proposer une fonctionnalité

## 🎯 À Propos

Ce Wiki est maintenu par la communauté. N'hésitez pas à :
- Créer de nouvelles pages
- Améliorer le contenu existant
- Partager vos expériences
- Ajouter des astuces

## 📝 Comment Contribuer

1. Cliquer sur **"Edit"** en haut de la page
2. Modifier le contenu
3. Sauvegarder avec un message de commit descriptif

---

**Merci de contribuer !** 🙏
