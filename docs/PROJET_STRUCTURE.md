...
**Contenu**:
- `frontal/Dockerfile` - Image Docker pour nœuds frontaux
...
- `packages/` - Emplacement pour les RPMs (Telegraf)
...
### 3. `gpfs/` - Configuration GPFS (optionnel)
...
**Contenu**:
- `matlab/` - Configuration MATLAB R2023b + DCS (optionnel)
...
**Contenu**:
- `install-all.sh` - Script d'installation principal
...
| `grafana-dashboards/` | `grafana-dashboards/` (racine) |
...
1. **Packages**: Les RPMs doivent être copiés manuellement dans `docker/packages/` avant le build si nécessaire
